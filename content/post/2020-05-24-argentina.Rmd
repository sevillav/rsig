...
Nuestro objetivo es mostrar el análisis estadístico realizado para conocer la precisión que tuvo el modelado fotogrametrico para generar el modelo digital de elevación (MDE) de PAE. Los indicadores empleados serán los errores, diferencias o residuos: medios (ME), medio absoluto (MAE), medio cuadrático (MSE), raíz cuadrada del error medio (RMSE), el tercer cuartil (ErrQ75) y el R^2^.
...
```{r}
dsm <- raster("C:/Proyectos/Diego/PAE/DEM_AG_VS.tif")
...
plot(puntos_sp$Arg_Prof_dsm, puntos_sp$AlturaZ, main="Relacion entre alturas modeladas por fotogrametria y alturas reales ", xlab="Altura modelada (MDE)", 
     ylab='Altura reales de pts de control', pch= 16, col = "blue", text(15,0.5, "TEXT"))
abline(0,1, lty=2, col='red')
abline(lm(puntos_sp$AlturaZ ~ puntos_sp$Arg_Prof_dsm), col = 'blue', lty=2)
legend(x = 241, y = 253,  legend = c("Relacion ideal 1:1", "Fotogrametria"), col = c("Red", "Blue"), 
       title = "Leyenda", lty = 2, cex = 0.75)
text(252,242,"R2 : 0.99", cex = 1.2)
```
...
Las diferencias del modelo en general son muy bajas, con un error absoluto promedio de 4 mm, un error cuadrático de 20 cm y una coincidencia entre valores de alturas reales y modelados muy alta de 0,99. En definitiva la mayoría de los puntos no sobre pasan los 23 cm de diferencias.